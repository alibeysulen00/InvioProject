package com.example.invioproject.ViewModel

import androidx.lifecycle.ViewModel
import androidx.lifecycle.viewModelScope
import com.example.invioproject.Repository.MainRepository
...
constructor(private val mainRepository: MainRepository) : ViewModel() {
    ...
    fun getLocation(id: String) {
        viewModelScope.launch {
            postStateFlow.value = ApiState.Loading
            mainRepository.getLocation(id).catch { e ->
                postStateFlow.value = ApiState.Failure(e)
            }.collect { data ->
                postStateFlow.value = ApiState.LocationSucess(data.results)
            }
        }
    }
